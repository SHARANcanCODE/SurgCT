...
export function applyColormap3D(actor: any, colormap: Volume3DColormap, wl: { wc: number; ww: number }): void {
  if (!actor) return;
  const prop = actor.getProperty?.() ?? actor.getProperty;
  if (!prop) return;
  const lo = wl.wc - wl.ww / 2;
  const hi = wl.wc + wl.ww / 2;
  const ctf = vtkColorTransferFunction.newInstance();
  const stops = COLORMAPS[colormap] ?? COLORMAPS.grayscale;
  for (const [tt, r, g, b] of stops) ctf.addRGBPoint(lo + (hi - lo) * tt, r, g, b);
