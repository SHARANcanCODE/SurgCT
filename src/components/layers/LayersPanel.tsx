...
function IconButton({
  ...
  return (
    <button
      ...
      title={title}
      className={`
        w-6 h-6 flex items-center justify-center rounded-none transition-colors
        ${disabled
...
function LayerRow({ name, visible, active = false, onToggleVisible, onEdit, onDelete, onRename, onSelect }: LayerRowProps) {
  ...
  return (
    <div
      className={`
        flex items-center gap-1 px-2 py-1.5 rounded-none border
        ${active
    ...
    >
      {renaming ? (
        <input
          ...
          className="flex-1 min-w-0 bg-white text-gray-800 dark:bg-gray-900 dark:text-gray-200 text-xs rounded-none px-1 py-0.5 border border-dental-500 outline-none"
...
export function LayersContent() {
  ...
  return (
    <div className="space-y-1">
          ...
          {state.scans.map(sc => (
            <div key={sc.id} className="rounded-none border border-transparent px-1 pb-1">
              <LayerRow
                name={sc.name}
                visible={sc.visible}
              ...
              <div className="flex items-center gap-2 px-2 pt-0.5">
                <select
                  ...
                  className="flex-1 min-w-0 bg-gray-100 text-gray-700 border-gray-300 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-600 text-[11px] rounded-none px-1 py-0.5 border"
                >
                  {SCAN_TYPES.map(ty => (
                ...
                <input
                  ...
                  className="h-5 w-6 rounded-none border border-gray-300 dark:border-gray-600 bg-transparent cursor-pointer"
              ...
              <div className="flex items-center gap-1.5 mt-1 ml-2">
                <button
                  ...
                  className="px-2 py-0.5 text-[11px] rounded-none bg-gray-200 text-gray-700 hover:bg-gray-300 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600 transition-colors"
                >
                  {t('reg.start')}
                </button>
                {sc.type === 'toothSetup' && (
                  <button
                    ...
                    className="px-2 py-0.5 text-[11px] rounded-none bg-dental-600 text-white hover:bg-dental-700 transition-colors"
                  >
                    {t('crown.plan')}
                  </button>
                )}
