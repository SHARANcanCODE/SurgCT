    ...
    <div className={`dcv-root ${theme === 'dark' ? 'dark' : ''} fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4`} onClick={onClose}>
      ...
      >
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {sections.views && (
              <div className="flex items-center gap-2 ml-auto">
                ...
                <input type="number" min={1} max={3} value={viewsCols} onChange={(e) => setViewsCols(Math.max(1, Math.min(3, Number(e.target.value))))} className="w-12 text-xs rounded-none border border-slate-300 dark:border-slate-600 bg-slate-100 dark:bg-slate-900 px-2 py-1" />
              </div>
            )}
          </div>
        </Section>
        ...
        <div className="flex justify-end gap-2 pt-1">
          ...
          <button onClick={doExport} disabled={busy} className="px-4 py-2 text-xs font-semibold rounded-none bg-dental-600 text-white hover:bg-dental-700 disabled:opacity-50">
