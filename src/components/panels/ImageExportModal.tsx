...
import { applyVolumeStyle } from '@/core/volume3DPreset';
...
export function ImageExportModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const seg = 'px-3 py-1 text-xs rounded-none border transition-colors';
  ...
  return createPortal(
    <div className={`dcv-root ${theme === 'dark' ? 'dark' : ''} fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4`} onClick={onClose}>
      <div
        className="w-full max-w-md max-h-[88vh] overflow-y-auto rounded-none bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-2xl p-5 space-y-4"
      ...
      >
        ...
        {has3D && (
          <Section title={t('imgexport.threeD')}>
            <div className="flex items-center gap-2">
              ...
              <select value={state.display.preset3d} onChange={(e) => set3DPreset(e.target.value)} className="flex-1 text-xs rounded-none border border-slate-300 dark:border-slate-600 bg-slate-100 dark:bg-slate-900 px-2 py-1">
          ...
          <div className="flex items-center gap-2">
            ...
            {outputMode === 'grid' && (
              <div className="flex items-center gap-2 ml-auto">
                ...
                <input type="number" min={1} max={4} value={gridCols} onChange={(e) => setGridCols(Math.max(1, Math.min(4, Number(e.target.value))))} className="w-12 text-xs rounded-none border border-slate-300 dark:border-slate-600 bg-slate-100 dark:bg-slate-900 px-2 py-1" />
              </div>
            )}
          </div>
        </Section>
        ...
        <div className="flex justify-end gap-2 pt-1">
          <button onClick={onClose} className="px-4 py-2 text-xs rounded-none border border-slate-300 dark:border-slate-600 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700">{t('common.cancel')}</button>
          <button
            onClick={doExport}
            disabled={busy || !Object.values(selected).some(Boolean)}
            className="px-4 py-2 text-xs font-semibold rounded-none bg-dental-600 text-white hover:bg-dental-700 disabled:opacity-50"
          >
