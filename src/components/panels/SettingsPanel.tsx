...
import { VOLUME_3D_COLORMAPS } from '@/core/volume3DPreset';
...
const FIELD =
  'w-full bg-slate-100 text-slate-800 border-slate-300 dark:bg-slate-800 dark:text-slate-200 dark:border-slate-600 text-xs rounded-none px-2 py-1.5 border outline-none focus:border-dental-500';
...
export function SettingsPanel() {
  ...
  const TABS: { id: Tab; label: string }[] = [
    ...
    { id: 'threed', label: t('settings.3d') },
    { id: 'patient', label: t('settings.patient') },
    { id: 'panoramic', label: t('settings.panoramic') },
    { id: 'implant', label: t('settings.implant') },
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4" onMouseDown={close}>
      ...
      <div
        className="relative flex w-full max-w-3xl h-[80vh] rounded-none overflow-hidden shadow-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900"
      ...
      >
        ...
        <div className="w-44 shrink-0 border-r border-slate-200 dark:border-slate-700/60 bg-slate-50 dark:bg-slate-900/60 p-2 space-y-1">
          ...
          {TABS.map((tb) => (
            <button
              ...
              className={`w-full text-left px-3 py-2 rounded-none text-xs transition-colors ${
                tab === tb.id
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5">
          <div className="flex items-center justify-between">
            ...
            <button onClick={close} title={t('layers.close')} className="w-7 h-7 flex items-center justify-center rounded-none text-slate-500 hover:bg-slate-200 dark:hover:bg-slate-800">✕</button>
          </div>
                ...
                <div className="flex gap-1">
                  {(['light', 'dark'] as const).map((th) => (
                    <button
                      ...
                      className={`flex-1 px-2 py-1.5 text-xs rounded-none transition-colors ${
                ...
                <div className="flex items-center gap-2 pt-1">
                  ...
                  {(['main', 'all'] as const).map((sc) => (
                    <button key={sc} onClick={() => setDisplay({ scope: sc })}
                      className={`px-2 py-1 text-[11px] rounded-none transition-colors ${d.scope === sc ? 'bg-dental-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700'}`}>
                      {t(`settings.scope.${sc}`)}
                    </button>
                  ))}
                </div>
              ...
              <Section title={t('settings.labels')}>
                ...
                <div className="flex items-center gap-3">
                  <label className="flex items-center gap-1.5 text-xs text-slate-600 dark:text-slate-300">
                    {t('safety.color')}
                    <input type="color" value={d.labelColor} onChange={(e) => setDisplay({ labelColor: e.target.value })} className="h-6 w-8 rounded-none border border-slate-300 dark:border-slate-600 bg-transparent cursor-pointer" />
                  </label>
                  <label className="flex items-center gap-1.5 text-xs text-slate-600 dark:text-slate-300">
                    {t('settings.labelMain')}
                    <input type="number" min={8} max={40} value={d.labelSizeMain} onChange={(e) => setDisplay({ labelSizeMain: Number(e.target.value) })} className="w-14 bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-none px-1.5 py-1 text-xs" />
                  </label>
                  <label className="flex items-center gap-1.5 text-xs text-slate-600 dark:text-slate-300">
                    {t('settings.labelSide')}
                    <input type="number" min={8} max={40} value={d.labelSizeSide} onChange={(e) => setDisplay({ labelSizeSide: Number(e.target.value) })} className="w-14 bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-none px-1.5 py-1 text-xs" />
                  </label>
                </div>
                <div className="flex gap-1">
                  {(['left', 'center', 'right'] as const).map((al) => (
                    <button key={al} onClick={() => setDisplay({ labelAlign: al })}
                      className={`flex-1 px-2 py-1 text-xs rounded-none transition-colors ${d.labelAlign === al ? 'bg-dental-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700'}`}>
                      {t(`settings.align.${al}`)}
                    </button>
                  ))}
                </div>
              ...
              <div className="flex gap-1">
                {(['AVG', 'MIP'] as ProjectionMode[]).map((mode) => (
                  <button key={mode} onClick={() => dispatch({ type: 'SET_PANORAMIC_PROJECTION', payload: mode })}
                    className={`flex-1 px-2 py-1 text-xs rounded-none transition-colors ${state.panoramicProjection === mode ? 'bg-dental-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700'}`}>
                    {mode}
                  </button>
                ))}
              </div>
          ...
          {tab === 'implant' && (
            <Section title={t('settings.safety')}>
              ...
              <label className="flex items-center justify-between">
                ...
                <input type="color" value={state.safety.color} onChange={(e) => dispatch({ type: 'SET_SAFETY', payload: { color: e.target.value } })} className="h-6 w-10 rounded-none border border-slate-300 dark:border-slate-600 bg-transparent cursor-pointer" />
              </label>
          ...
          {tab === 'about' && (
            <>
              <Section title="SURGCT Platform">
                <p className="text-sm font-semibold text-cyan-400">
                  SURGCT Clinical Imaging Suite
                </p>
                <p className="text-xs text-slate-400 mt-1">
                  SURGCT v{APP_VERSION} · Professional Dental CT &amp; AI Analytics
                </p>
              </Section>
