...
export function IntroTour() {
  ...
  return (
    <div className="fixed inset-0 z-[65] flex items-center justify-center p-4" onMouseDown={close}>
      ...
      <div
        className="relative w-full max-w-md rounded-none overflow-hidden shadow-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900"
      ...
      >
        ...
        <div className="flex justify-center gap-1.5 pb-4">
          {STEP_KEYS.map((_, idx) => (
            <button
              key={idx}
              onClick={() => setI(idx)}
              className={`h-1.5 rounded-none transition-all ${idx === i ? 'w-5 bg-dental-500' : 'w-1.5 bg-slate-300 dark:bg-slate-600'}`}
        ...
        <div className="flex items-center justify-between px-5 py-3 border-t border-slate-200 dark:border-slate-700/60">
          ...
          <div className="flex items-center gap-2">
            {i > 0 && (
              <button onClick={() => setI(i - 1)} className="px-3 py-1.5 rounded-none text-xs bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">
                {t('intro.back')}
              </button>
            )}
            <button
              onClick={() => (i === last ? close() : setI(i + 1))}
              className="px-4 py-1.5 rounded-none text-xs font-semibold bg-dental-600 text-white hover:bg-dental-700 transition-colors"
            >
