    ...
    <div className="flex flex-col gap-0.5 p-2">
      <div className="flex items-center justify-between px-1 mb-1.5">
        ...
        <button
          ...
          className="flex items-center gap-1 text-[11px] px-2 py-1 rounded-none border border-slate-700 text-slate-300 hover:bg-slate-800 hover:text-white transition-colors"
        >
      ...
      {state.studies.map((study) => {
        ...
        return (
          <div key={study.studyInstanceUID} className="flex flex-col">
            ...
            <div
              className={`group flex items-center gap-1 rounded-none pr-1 transition-colors ${
                isActiveStudy ? 'bg-slate-800/60' : 'hover:bg-slate-800/40'
              }`}
            >
              <button
                onClick={() => {
                ...
                className={`flex items-center gap-1.5 flex-1 min-w-0 text-left px-2 py-1.5 text-sm ${
                  isActiveStudy ? 'text-white font-semibold' : 'text-gray-300'
                }`}
              >
                ...
                <svg className="w-4 h-4 shrink-0 text-slate-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6">
              ...
              <button
                ...
                className="shrink-0 w-5 h-5 flex items-center justify-center rounded-none text-gray-400 hover:text-red-500 hover:bg-red-500/10 opacity-0 group-hover:opacity-100 focus:opacity-100 transition-opacity"
              >
            ...
            {!isCollapsed && study.series.map((series) => {
              ...
              return (
                <button
                  ...
                  className={`ml-4 text-left px-2.5 py-1.5 rounded-none text-sm transition-colors border ${
                    isActiveSeries
                      ? 'bg-slate-800 text-white border-slate-600'
                      : 'text-gray-300 hover:bg-slate-800/40 border-transparent'
                  }`}
                >
                  <div className="font-medium truncate">{series.seriesDescription || `Series #${series.seriesNumber}`}</div>
                  <div className="text-[11px] text-gray-400 mt-0.5">{series.modality} · {t('series.images', { n: series.imageCount })}</div>
                  {geom && <div className="text-[10px] text-gray-500">{geom}</div>}
                </button>
              );
            })}
          </div>
