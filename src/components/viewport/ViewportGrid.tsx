...
export function ViewportGrid() {
  ...
  if (state.layoutMode === '2x2') {
    return (
      <div className="grid grid-cols-2 grid-rows-2 w-full h-full gap-px bg-gray-700">
        <div className="relative w-full h-full overflow-hidden min-w-0 min-h-0"><ViewportMPR orientation="AXIAL" volumeId={vid} /></div>
        <div className="relative w-full h-full overflow-hidden min-w-0 min-h-0"><ViewportMPR orientation="SAGITTAL" volumeId={vid} /></div>
        <div className="relative w-full h-full overflow-hidden min-w-0 min-h-0"><ViewportMPR orientation="CORONAL" volumeId={vid} /></div>
        <div className="relative w-full h-full overflow-hidden min-w-0 min-h-0"><Viewport3D volumeId={vid} /></div>
      </div>
    );
  }
  ...
  if (state.layoutMode === '1+3') {
    ...
    if (grid === '2x2') {
      ...
      return (
        <div className="grid grid-cols-2 grid-rows-2 w-full h-full gap-px bg-gray-700">
          {four.map((k) => <div key={k} className="relative w-full h-full overflow-hidden min-w-0 min-h-0">{renderView(k)}</div>)}
        </div>
      );
    }
    ...
    if (arrangement === 'top') {
      return (
        <div className="flex flex-col w-full h-full gap-px bg-gray-700">
          <div key={big} className="flex-1 min-h-0 relative overflow-hidden">{renderView(big)}</div>
          <div className="h-1/3 flex gap-px min-h-0">
            {small.map((k) => <div key={k} className="flex-1 min-w-0 relative overflow-hidden">{renderView(k)}</div>)}
          </div>
        </div>
      );
    }
    return (
      <div className="flex w-full h-full gap-px bg-gray-700">
        <div key={big} className="flex-1 h-full min-w-0 relative overflow-hidden">{renderView(big)}</div>
        <div className="w-1/3 h-full flex flex-col gap-px">
          {small.map((k) => <div key={k} className="flex-1 min-h-0 relative overflow-hidden">{renderView(k)}</div>)}
        </div>
      </div>
    );
  }
  ...
  if (state.layoutMode === 'OPG2+1') {
    const smallNodes = [
      <div key="ax" className="relative flex-1 min-w-0 min-h-0 overflow-hidden">
        <ViewportMPR orientation="AXIAL" volumeId={vid} />
        <ArchCurveEditor />
        <ImplantAxialOverlay />
      </div>,
      <div key="cs" className="relative flex-1 min-w-0 min-h-0 overflow-hidden">
        <ViewportCrossSection volumeId={vid} />
      </div>,
      <div key="co" className="relative flex-1 min-w-0 min-h-0 overflow-hidden">
    ...
    if (state.panel.panoArrangement === 'left') {
      return (
        <div className="flex w-full h-full gap-px bg-gray-700">
          <div className="flex-1 min-w-0 relative overflow-hidden">{pano}</div>
    ...
    return (
      <div className="flex flex-col w-full h-full gap-px bg-gray-700">
        <div className="flex-1 min-h-0 relative overflow-hidden">{pano}</div>
