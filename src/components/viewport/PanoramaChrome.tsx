...
function PlanButton({ active, label, title, onClick }: {
  ...
  return (
    <button
      onClick={onClick}
      title={title}
      className={`px-2 py-1 rounded-none text-[11px] font-medium transition-colors ${
        active ? 'bg-dental-600 text-white' : 'text-slate-200 hover:bg-slate-700/60'
      }`}
    >
      {label}
...
export function PanoramaChrome() {
  ...
  return (
    <div className="absolute top-2 right-2 z-30 flex flex-col items-end gap-2">
      {/* Tools box */}
      <div className="flex flex-col gap-1 rounded-none bg-slate-900/70 backdrop-blur-sm border border-slate-700/60 px-1.5 py-1 shadow-lg">
      ...
      <div className="flex flex-col gap-1 rounded-none bg-slate-900/70 backdrop-blur-sm border border-slate-700/60 px-1.5 py-1 shadow-lg">
        ...
        <div className="flex items-center gap-1">
          ...
          />
          <PlanButton
            active={state.anatomyDrawMode === 'nerve'}
            label={t('anatomy.nerve')}
