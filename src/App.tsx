...
import { SurgctAIChat } from '@/components/chat/SurgctAIChat';
...
function ViewerApp({
  ...
  return (
    <div className={`dcv-root ${theme === 'dark' ? 'dark' : ''} ${props.className ?? ''} h-full w-full`}>
      <div className="flex flex-col h-full w-full overflow-hidden bg-[#080d1a] text-slate-100 dark:bg-[#080d1a] dark:text-slate-100">
        {state.study && <TopBar />}
...
/**
 * Embeddable SURGCT (Surgical CBCT & AI Analytics Viewer).
 */
